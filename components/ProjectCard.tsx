  ...
  return (
    <div className="bg-secondary h-full flex flex-col rounded-lg overflow-hidden shadow-lg shadow-black/30 transform transition-all duration-300 hover:shadow-accent/20 hover:-translate-y-2" style={{ height: '420px' }}>
      ...
      <div className="p-6 flex flex-col flex-grow">
        ...
        <p className="text-text-secondary mb-4 text-sm flex-grow">{project.description}</p>
        <div className="flex flex-wrap gap-2 mb-4">
          {project.tags.map((tag) => (
            <SkillBadge key={tag} skill={tag} />
          ))}
        </div>
        <div className="flex items-center space-x-4 mt-auto pt-4 border-t border-primary">
          {project.liveUrl && (
            <a href={project.liveUrl} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline flex items-center gap-1 font-medium">
              Live Demo
              <svg xmlns="http://www.w3.org/2000/svg" className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 6H6a2 2 0 00-2 2v10a2 2 0 002 2h10a2 2 0 002-2v-4M14 4h6m0 0v6m0-6L10 14" /></svg>
            </a>
          )}
          {project.githubUrl && (
             <a href={project.githubUrl} target="_blank" rel="noopener noreferrer" className="text-accent hover:underline flex items-center gap-1 font-medium">
              GitHub
